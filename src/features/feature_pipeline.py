...
from features.micro_price import MicroPriceCalculator, compute_micro_price_features
from features.volume_profiles import VolumeProfileCalculator, compute_volume_features
from features.queue_dynamics import compute_queue_metrics_from_snapshots
...
class FeaturePipeline:
    ...
    def compute_all_features(
        ...
        df = compute_ofi_from_dataframe(
            df, levels=self.config.ofi_levels, window_sizes=self.config.ofi_windows
        )
        ...
        df = compute_volume_features(df, depth_levels=self.config.volume_depth_levels)

        print("Computing Queue Dynamics...")
        queue_metrics = compute_queue_metrics_from_snapshots(
            df.to_dict("records"), window_size=self.config.queue_window_size
        )
        ...
        if include_volatility:
            ...
            ohlc_df = compute_ohlc_from_snapshots(
                df.to_dict("records"), window_ticks=self.config.ohlc_bar_size
            )

            vol_df = compute_volatility_features(
                ohlc_df, windows=self.config.volatility_windows
            )
            ...
            for col in vol_df.columns:
                if col not in ["timestamp", "open", "high", "low", "close"]:
                    ...
                    repeated_values = np.repeat(
                        vol_df[col].values, self.config.ohlc_bar_size
                    )
...
def create_training_dataset(
    ...
    features_df["future_mid_price"] = features_df["mid_price"].shift(
        -prediction_horizon
    )
...
if __name__ == "__main__":
    ...
    dataset = create_training_dataset(
        features_df, prediction_horizon=10, threshold_bps=5.0, sequence_length=50
    )
