...
class EconomicValidator:
    ...
    def validate_strategy(self,
                         ...
                         volume_df: Optional[pd.DataFrame] = None) -> EconomicMetrics:
        """
    ...
    def _simulate_realistic_execution(self, df: pd.DataFrame) -> List[Dict]:
        ...
        for idx, row in df.iterrows():
            ...
            if position is not None and signal != position['direction']:
                exit_price, exit_cost = self._calculate_execution_price(
                    -position['direction'], mid_price, bid, ask,
                ...
                position['pnl'] = pnl
                position['holding_period'] = (timestamp - position['entry_timestamp']).total_seconds()
        ...
        if position is not None:
            ...
            position['exit_cost_bps'] = exit_cost
            position['pnl'] = pnl
            position['holding_period'] = (final_row['timestamp'] - position['entry_timestamp']).total_seconds()
    ...
    def _calculate_execution_price(self,
        ...
        impact_bps = self.market_conditions.impact_coefficient * np.sqrt(trade_size / volume) * 10000
    ...
    def _calculate_deflated_sharpe(self, returns: pd.Series, n_trials: int = 10) -> float:
        """
    ...
    def generate_report(self, metrics: EconomicMetrics) -> str:
        ...
        report.append(f"  Transaction Cost Drag: {metrics.transaction_cost_drag:>8.2f}%")
        ...
        report.append(f"  Drawdown Duration:     {metrics.drawdown_duration:>8d} periods")
        ...
        report.append(f"  Avg Holding Period:    {metrics.avg_holding_period:>8.0f} seconds")

        report.append("\n[ECONOMIC VIABILITY]")
        report.append(f"  Breakeven Cost:        {metrics.breakeven_cost_bps:>8.2f} bps")
        ...
        # Overall assessment
        if metrics.is_significant and metrics.cost_capacity > 0 and metrics.net_return > 0:
