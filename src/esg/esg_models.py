...
class ESGScorer:
    ...
    def __init__(self):
        ...
        self.weights = {"environmental": 0.35, "social": 0.35, "governance": 0.30}
    ...
    def calculate_environmental_score(self, metrics: ESGMetrics) -> Dict:
        ...
        co2_score = max(
            0, 100 - (co2_intensity / self.benchmarks["co2_intensity"]) * 100
        )

        # Water intensity (lower is better)
        water_intensity = metrics.water_usage / (metrics.revenue + 1)
        water_score = max(
            0, 100 - (water_intensity / self.benchmarks["water_intensity"]) * 100
        )
    ...
    def calculate_governance_score(self, metrics: ESGMetrics) -> Dict:
        ...
        comp_score = max(
            0, 100 - ((comp_ratio - benchmark_ratio) / benchmark_ratio) * 50
        )
