...
def create_esg_scorecard(company: ESGMetrics):
    ...
    with col1:
        ...
        st.markdown(f"**Sector:** {company.sector} | **Market Cap:** ${company.market_cap:,.0f}M")
    ...
    categories = ['CO2 Efficiency', 'Water Usage', 'Renewable Energy',
    ...
    categories_social = ['Employee Satisfaction', 'Diversity', 'Safety',
                         'Community Investment', 'Supply Chain Ethics']
    values_social = [
    ...
    categories_gov = ['Board Independence', 'Gender Diversity',
                      'Fair Compensation', 'Audit Quality', 'Shareholder Rights']
    values_gov = [
...
def create_risk_return_viz():
    ...
    for company in companies:
        ...
        base_return = 8
        esg_premium = (result['overall_score'] - 50) / 10 * 2  # 2% per 10 points above 50
        expected_return = base_return + esg_premium + np.random.randn() * 1

        # Synthetic volatility (ESG leaders tend to have lower risk)
        base_vol = 20
        esg_risk_reduction = (result['overall_score'] - 50) / 10 * 1.5  # -1.5% vol per 10 points
    ...
    efficient_return = df['expected_return'].max() - 0.01 * (vol_range - df['volatility'].min())**1.5
...
def create_sentiment_alerts():
    ...
    if not filtered_alerts:
        ...
        for alert in sorted(filtered_alerts, key=lambda x: (x['severity'] != 'HIGH', x['timestamp']), reverse=True):
...
def create_whatif_simulator():
    ...
    with col1:
        ...
        renewable_change = st.slider(
            "Renewable Energy Change (percentage points)",
            -30, 30, 0, 5
        )

        waste_change = st.slider(
            "Waste Recycling Change (percentage points)",
            -30, 30, 0, 5
        )
    ...
    with col2:
        ...
        satisfaction_change = st.slider(
            "Employee Satisfaction Change (points)",
            -20, 20, 0, 2
        )

        diversity_change = st.slider(
            "Diversity Score Change (points)",
            -20, 20, 0, 2
        )

        board_diversity_change = st.slider(
            "Female Board Members Change (percentage points)",
            -20, 20, 0, 2
        )
    ...
    modified_company = ESGMetrics(
        ...
        renewable_energy_pct=min(100, max(0, company.renewable_energy_pct + renewable_change)),
        ...
        employee_satisfaction=min(100, max(0, company.employee_satisfaction + satisfaction_change)),
        ...
        female_board_members=min(100, max(0, company.female_board_members + board_diversity_change)),
    ...
    env_delta = modified_result['pillar_scores']['environmental'] - baseline_result['pillar_scores']['environmental']
    social_delta = modified_result['pillar_scores']['social'] - baseline_result['pillar_scores']['social']
    gov_delta = modified_result['pillar_scores']['governance'] - baseline_result['pillar_scores']['governance']
    ...
    if rating_changed:
        st.success(f"🎯 Rating changed: {baseline_result['rating']} → {modified_result['rating']}")
...
def main():
    ...
    # Header
    st.markdown('<div class="main-header">🌍 ESG Analytics Dashboard</div>', unsafe_allow_html=True)

    # Sidebar
    with st.sidebar:
        st.image("https://via.placeholder.com/300x100/4CAF50/FFFFFF?text=ESG+Analytics", use_column_width=True)
    ...
    if page == "🏢 Company Scorecards":
        ...
        with col1:
            sector_scores = df.groupby('Sector')['ESG Score'].mean().sort_values(ascending=False)
