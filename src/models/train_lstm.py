...
from features.feature_pipeline import FeaturePipeline, FeaturePipelineConfig, create_training_dataset
...
class Trainer:
    ...
    def prepare_data(self, features_df):
        ...
        X_train, X_val, y_train, y_val = train_test_split(
            X_temp, y_temp,
            test_size=self.config['training']['val_split'] / (1 - self.config['training']['test_split']),
            random_state=42,
            stratify=y_temp
        )
...
def main():
    parser = argparse.ArgumentParser(description='Train LSTM for Order Book Forecasting')
    parser.add_argument('--config', type=str, default='../configs/models/lstm_config.yaml',
    ...
    if args.data:
        ...
        for i in range(2000):
            ...
            bids = [[mid_price - (j+1)*0.5, 50 + np.random.normal(0, 10)] for j in range(20)]
            asks = [[mid_price + (j+1)*0.5, 50 + np.random.normal(0, 10)] for j in range(20)]
    ...
    # Prepare data
    train_loader, val_loader, test_loader, scaler, feature_names = trainer.prepare_data(features_df)
