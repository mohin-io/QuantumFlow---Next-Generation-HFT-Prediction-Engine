...
class LiveDataAggregator:
    ...
    def get_consolidated_book(self, symbol_map: Dict[str, str], depth: int = 10) -> pd.DataFrame:
        """
    ...
    def calculate_arbitrage_opportunities(self, symbol_map: Dict[str, str]) -> List[Dict]:
        """
...
if __name__ == '__main__':
    ...
    if btc_book:
        ...
        print(f"   Best Bid: ${btc_book.bids[0][0]:,.2f} ({btc_book.bids[0][1]:.4f} BTC)")
        print(f"   Best Ask: ${btc_book.asks[0][0]:,.2f} ({btc_book.asks[0][1]:.4f} BTC)")
